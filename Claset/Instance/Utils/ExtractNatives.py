...
def extractNatives(VersionJson: dict, ExtractTo: str, Features: dict | None = None):
    """从 Native.jar 文件提取 Natives"""
    for Libraries in VersionJson["libraries"]:
        NativesObject = getNativesObject(
            Libraries=Libraries, Features=Features, getExtract=True
        )
        if NativesObject is None:
            continue
        ...
        for FilePathInZip in FileListBackup:
            ...
            if Ext == ".git":
                ...
                Logger.debug(
                    'Excluded: "%s" From "%s" By file extension name',
                    FilePathInZip,
                    BaseNameByPath,
                )
            elif Ext == ".sha1":
        ...
        for FilePathInZip in FileList:
            ...
            if dfCheck(Path=RealFilePath, Type="f"):
                if FilePathInZip in FileSha1:
                    if (
                        sha1(loadFile(Path=RealFilePath, Type=FileTypes.Bytes)).hexdigest()
