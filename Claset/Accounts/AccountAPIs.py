...
class MicrosoftAuthAPI:
    ...
    def __init__(
        self,
        MicrosoftAccountRefreshToken: str | None = None,
        MicrosoftAccountAccessToken: str | None = None,
    ):
    ...
    def auth(self):
        ...
        while True:
            ...
            match CheckRequestReturned.get("error"):
                ...
                case "authorization_declined":
                    raise MicrosoftOAuthDeclined
                case "expired_token":
                    raise MicrosoftOAuthTimeOut
                case None:
                    break
                case _:
                    raise ValueError(
                        CheckRequestReturned.get("error"),
                        CheckRequestReturned.get("error_description"),
                    )
        ...
        self.MicrosoftAccountAccessTokenExpiresTime = (
            CheckRequestReturned["expires_in"] + int(time()) - 5
        )
    ...
    def refresh(self, MicrosoftAccountRefreshToken: str | None = None):
        ...
        if MicrosoftAccountRefreshToken is None:
            if self.MicrosoftAccountRefreshToken is None:
                raise ValueError
            MicrosoftAccountRefreshToken = self.MicrosoftAccountRefreshToken

        (
            self.MicrosoftAccountAccessToken,
            self.MicrosoftAccountRefreshToken,
            self.MicrosoftAccountAccessTokenExpiresTime,
        ) = self.refreshAccessTokens(MicrosoftAccountRefreshToken)

    def refreshAccessTokens(
        self, MicrosoftAccountRefreshToken: str | None = None
    ) -> tuple[str, str, str]:
...
class MinecraftAccountAPI:
    ...
    def getAccountInfos(
        self, MinecraftAccessToken: str | None = None
    ) -> tuple[UUID, str, list, list]:
        """获取账户相关信息"""
