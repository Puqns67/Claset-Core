...
class AccountManager:
    ...
    def create(self, Type: str = "OFFLINE", Name: str | None = None) -> dict[str, str | int]:
        ...
        match Type:
            ...
            case "MICROSOFT":
                ...
                ] = MicrosoftOAuthTask.MicrosoftAccountAccessToken
                NewAccount[
                    "MicrosoftAccountRefreshToken"
                ] = MicrosoftOAuthTask.MicrosoftAccountRefreshToken
                NewAccount[
                    "MicrosoftAccountAccessTokenExpiresTime"
                ] = MicrosoftOAuthTask.MicrosoftAccountAccessTokenExpiresTime
            case _:
    ...
    def remove(self, ID: int) -> None:
        ...
        match self.Configs["Accounts"][ID]["Status"]:
            ...
            case "DELETE":
                Logger.warning(
                    "Account ({}) deleted, Skipping it".format(
                        self.Configs["Accounts"][ID]["Name"]
                    )
                )
                return
    ...
    def setDefault(self, ID: int, RaiseException=True) -> None:
        ...
        if (ID <= -1) or (len(self.Configs["Accounts"]) - 1 < ID):
            if RaiseException:
                ...
                Logger.warning(
                    "Account ({})  not found, Skipping it".format(
                        self.Configs["Accounts"][ID]["Name"]
                    )
                )
                return
    ...
    def getAccountObject(self, ID: int | None = None) -> Account:
        ...
        if ID not in Accounts:
            Accounts[ID] = Account(Account=self.Configs["Accounts"][ID], Manager=self)
        return Accounts[ID]

    def getAccountList(
        self,
        ID: int | None = None,
        UUID: Class_UUID | str | None = None,
        Name: str | None = None,
        Type: str | None = None,
    ) -> list[dict]:
        """获取以各种方式过滤后的用户列表"""
